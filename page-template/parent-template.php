...
<?php
    $tenant_retention_solution = get_field('tenant_retention_solution');

    if($tenant_retention_solution):

    $tenant_retention_section_title = $tenant_retention_solution['section_title'];
    $tenant_retention_solution_list = $tenant_retention_solution['package_list'];
?>
<section class="sb-tenant-retention">
    <div class="container">
        <div class="sb-section-title text-center">
            <h2><?php echo wp_kses_post( $tenant_retention_section_title['title'] ?? '' ); ?></h2>
            <h4><?php echo wp_kses_post( $tenant_retention_section_title['sub_title'] ?? ''); ?></h4>
            <?php if($tenant_retention_section_title['description']): ?>
            <p><?php echo wp_kses_post( $tenant_retention_section_title['description'] ?? '' ); ?></p>
            <?php endif; ?>
        </div>
        <div class="retention-solution-list d-flex flex-wrap space-between">

            <?php
                if($tenant_retention_solution_list):

                    foreach($tenant_retention_solution_list as $list):

                        $tenant_retention_img_position = $list['image_alignment'];
            ?>
            <div class="sb-card sb-service-card image-position-<?php echo esc_attr( $tenant_retention_img_position ?? '' ); ?>"
...
<?php
$parent_booking_cta = get_field('parent_booking_cta');
$parent_booking_cta_switch = $parent_booking_cta['booking_cta_switch'];

if($parent_booking_cta_switch):
